...
function ProfileInfoModal ({userName,isUser, profilePicture,  cakeDate}) {
        ...
        async function cookiesfn() {
          ...
          if(cookies&&cookies.username&&cookies.access_token){
            ...
            if(cookies.username !== userName){
                setLoading(true);
                const response=await handler(`/users/isfollowed/${userName}`,"GET", "",cookies.access_token);
    ...
    return (
        <div className={styles.modal} onClick={(e) => {e.stopPropagation();}} >
            {!loading&&
            <div>
            <div className={styles.nameAndPicture} onClick={() => {router.push(`/profile/${userName}`)}}>
