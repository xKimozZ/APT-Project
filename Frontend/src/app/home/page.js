...
function FileExplorer() {
  ...
  async function deleteFile(fileId) {
    ...
    if (!await validateSession())
      return;
    try {
      const sent = await apiHandler(`/documents/delete-file`, "POST", bodyData, token);
      console.log(sent);
      getFiles();
    } catch (error) {
      window.alert("You are not allowed to delete this document!");
      console.error("Error fetching data:", error);
    }
  }
  ...
  async function changePermission(
    ...
    if (!await validateSession())
      return;
    try {
      const sent = await apiHandler(
        `/documents/change-permissions`,
        "POST",
        bodyData,
        token
      );
      ...
      window.alert("You can't change your own permissions");
      console.error("Error fetching data:", error);
    }
  }
  ...
  async function confirmFileChanges(fileId, title, sharingOptionsOn, isPublic) {
    ...
    if (!await validateSession())
      return;
    try {
      const sent = await apiHandler(`/documents/file-change`, "POST", bodyData, token);
      console.log(sent);
      getFiles();
    } catch (error) {
        ...
        <div className={`${styles.allPadding} ${styles.container}`}>
          <div className={styles.header}>
            <div className={styles.toolbar}>
              ...
              >
                <OutlineButton isDanger={true} btnClick={handleLogout}>
                  <svg
